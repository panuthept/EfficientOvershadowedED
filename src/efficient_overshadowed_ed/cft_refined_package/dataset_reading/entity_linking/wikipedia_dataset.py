...
from refined_package.data_types.modelling_types import BatchedElementsTns
...
from refined_package.utilities.dataset_utils import mask_mentions
from refined_package.utilities.general_utils import correct_spans, merge_spans, get_logger
from refined_package.utilities.preprocessing_utils import convert_batch_elements_to_batched_tns
...
class WikipediaDataset(torch.utils.data.IterableDataset):
    ...
    def __init__(
        ...
        self.file_line_count = 6185825  # hard-coded for now to avoid running wc -l on a 50 GB file
        ...
        with open(self.dataset_path, "r") as dataset_file:
            ...
            for current_line_num, dataset_line in enumerate(dataset_file):
                ...
                if "predicted_spans" in parsed_line:
                    md_spans = [
                        Span(start=start, ln=end - start, is_eval=True, text=text, coarse_type=coarse_type)
