...
from efficient_overshadowed_ed.cft_refined_package.data_types.doc_types import (
    Doc,
)
from efficient_overshadowed_ed.cft_refined_package.data_types.modelling_types import BatchElementTns, BatchedElementsTns, BatchElement
from efficient_overshadowed_ed.cft_refined_package.doc_preprocessing.preprocessor import Preprocessor
from efficient_overshadowed_ed.cft_refined_package.model_components.mention_detection_layer import IGNORE_INDEX
from efficient_overshadowed_ed.cft_refined_package.utilities.general_utils import batch_items
...
def convert_batch_element_to_tensors(
    ...
    # Create token_id_values for entity mask augmentation view
    aug_token_id_values = torch.where(ner_labels == 0, token_id_values, processor.mask_id)
...
def collate_batch_elements_tns(
    ...
    b_aug_token_id_values = torch.zeros(
        (
            batch_size,
            b_token_ln,
        ),
    ...
    b_aug_token_id_values.fill_(token_pad_value)
    ...
    for item_idx, batch in enumerate(batch_elements_tns):
        ...
        if batch.aug_token_id_values is not None:
            b_aug_token_id_values[item_idx, :token_ln] = batch.aug_token_id_values
