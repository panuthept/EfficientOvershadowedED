...
from refined_package.dataset_reading.entity_linking.wikipedia_dataset import WikipediaDataset
from refined_package.doc_preprocessing.preprocessor import PreprocessorInferenceOnly
...
from refined_package.model_components.config import NER_TAG_TO_IX, ModelConfig
from refined_package.model_components.refined_model import RefinedModel
...
from refined_package.torch_overrides.data_parallel_refined import DataParallelReFinED
from refined_package.training.fine_tune.fine_tune import run_fine_tuning_loops
from refined_package.training.train.training_args import parse_training_args
...
def main():
    ...
    training_dataset = WikipediaDataset(
        # start=100,
        start=training_args.start_train_line,
        end=6185825,  # large number means every line will be read until the end of the file
    ...
    model = RefinedModel(
        ModelConfig(
            ...
            beta_desc=training_args.beta_desc,
            beta_et=training_args.beta_et,
            beta_ed=training_args.beta_ed,
        ),
        preprocessor=preprocessor
    )
