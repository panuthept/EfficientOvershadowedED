...
from efficient_overshadowed_ed.cft_refined_package.utilities.md_dataset_utils import bio_to_offset_pairs
from efficient_overshadowed_ed.cft_refined_package.data_types.modelling_types import BatchElement, BatchedElementsTns, ModelReturn
from efficient_overshadowed_ed.cft_refined_package.data_types.base_types import Span
from efficient_overshadowed_ed.cft_refined_package.doc_preprocessing.preprocessor import Preprocessor
from efficient_overshadowed_ed.cft_refined_package.utilities.preprocessing_utils import pad
from efficient_overshadowed_ed.cft_refined_package.model_components.config import ModelConfig
from efficient_overshadowed_ed.cft_refined_package.model_components.ed_layer_2 import EDLayer
from efficient_overshadowed_ed.cft_refined_package.model_components.entity_disambiguation_layer import EntityDisambiguation
from efficient_overshadowed_ed.cft_refined_package.model_components.entity_typing_layer import EntityTyping
from efficient_overshadowed_ed.cft_refined_package.model_components.mention_detection_layer import MentionDetection
from efficient_overshadowed_ed.cft_refined_package.utilities.model_utils import fill_tensor
from efficient_overshadowed_ed.cft_refined_package.utilities.general_utils import get_logger
...
class RefinedModel(nn.Module):
    ...
    def __init__(
        ...
        self.beta_desc = config.beta_desc
        self.beta_et = config.beta_et
        self.beta_ed = config.beta_ed
    ...
    def forward(
        ...
        if batch.aug_token_id_values is not None:
            aug_output: BaseModelOutputWithPoolingAndCrossAttentions = self.transformer(
                input_ids=batch.aug_token_id_values,
                attention_mask=batch.attention_mask_values,
                token_type_ids=batch.token_type_values,
                position_ids=None,
                head_mask=None,
            ...
            aug_contextualised_embeddings = aug_output.last_hidden_state
        ...
        if aug_contextualised_embeddings is not None:
            aug_mention_embeddings = self._get_mention_embeddings(
                sequence_output=aug_contextualised_embeddings,
                token_acc_sums=token_acc_sums,
                entity_mask=entity_mask,
            )

        # candidate_description_scores.shape = (num_ents, num_cands)
        description_loss, candidate_description_scores, cand_desc_emb = self.ed_2(
        ...
        combined_description_loss = None
        if aug_mention_embeddings is not None:
            aug_description_loss, aug_candidate_description_scores, _ = self.ed_2(
                candidate_desc=cand_desc,
                mention_embeddings=aug_mention_embeddings,
                candidate_entity_targets=candidate_entity_targets,
                candidate_desc_emb=cand_desc_emb,
            )
            combined_description_loss = description_loss + (self.beta_desc * aug_description_loss) if aug_description_loss is not None else description_loss
        ...
        combined_et_loss = None
        if aug_mention_embeddings is not None:
            aug_et_loss, aug_et_activations = self.entity_typing(
                mention_embeddings=aug_mention_embeddings, span_classes=class_targets
            )
            combined_et_loss = et_loss + (self.beta_et * aug_et_loss) if aug_et_loss is not None else et_loss
        ...
        combined_ed_loss = None
        if aug_candidate_description_scores is not None and aug_et_activations is not None:
            aug_ed_loss, aug_ed_activations = self.entity_disambiguation(
                class_activations=aug_et_activations.detach() if self.detach_ed_layer else aug_et_activations,
                candidate_entity_targets=candidate_entity_targets,
                candidate_pem_values=candidate_pem_values,
                candidate_classes=candidate_classes,
                candidate_description_scores=aug_candidate_description_scores.detach(),  # detach or not
                current_device=current_device,
            )
            combined_ed_loss = ed_loss + (self.beta_ed * aug_ed_loss) if aug_ed_loss is not None else ed_loss
